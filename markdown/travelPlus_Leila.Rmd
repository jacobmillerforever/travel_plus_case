...
```
## Q1(a) Overall purchase rate
```{r Q1a}
# Recode Purchase so it shows "No" and "Yes"
df <- df %>%
  mutate(Purchase = factor(Purchase, levels = c(0,1), labels = c("No","Yes")))

# Counts of No vs Yes
tab_purchase <- table(df$Purchase)

# Neat table with counts + percents
library(scales)
rate_tbl <- tibble::tibble(
  Purchase = names(tab_purchase),
  Count    = as.integer(tab_purchase),
  Percent  = percent(as.numeric(tab_purchase)/sum(tab_purchase), accuracy = 0.1)
)

knitr::kable(rate_tbl, caption = "Q1(a) Overall purchase rate")

# Summary sentence
overall_rate <- as.numeric(tab_purchase["Yes"])/sum(tab_purchase)
cat(sprintf("Overall purchase rate: %s (Yes out of all customers).",
            scales::percent(overall_rate, accuracy = 0.1)))
```

##From the results above, we see that **only 8% of customers purchased RoadsidePlus** (1,595 out of 20,000), while **92% did not purchase**.This shows the dataset is **highly imbalanced**, which is an important factor to consider for predictive modeling.
...
ggplot(df_long, aes(x = Purchase, y = value, fill = Purchase)) +
  ...
  theme_minimal() + theme(legend.position = "none")
knitr::kable(num_summary, caption = "Q1(b) Numeric: summary stats by Purchase")
```
##From the boxplots we can see that purchasers generally have **higher EmailClicks and EmailOpens** than non-purchasers.  Similarly, **PriorAddons, Score_Engagement, and Score_Upsell** are higher on average among purchasers.  This suggests that these numeric variables are predictive signals.  At the same time, the overlap across groups shows that some predictors may include noise.
...
legend("bottomright",
       ...
       col = c("blue", "green", "red", "pink", "black"), lwd = 2, lty = 1, cex=0.8 )
